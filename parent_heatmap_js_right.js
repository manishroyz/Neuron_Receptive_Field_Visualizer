let ct_array_rt = new Array();

function compute_ct(d){
    //console.log('-----------------------');
    let rf1pf1 = Math.sqrt(Math.pow((d.cell_id_x-fp1_x_rt),2) +  Math.pow((d.cell_id_y-fp1_y_rt),2));
    let rf1pf2 = Math.sqrt(Math.pow((d.cell_id_x-fp2_x_rt),2) +  Math.pow((d.cell_id_y-fp2_y_rt),2));
    //console.log('RF1FP1: '+ rf1pf1 + ' AND ' + 'RF1FP2: '+ rf1pf2);
    let ind = (rf1pf2 - rf1pf1)/(rf1pf1 + rf1pf2);
    //console.log('ind-->'+ind);
    // Setting saccade direction :
    let sacc;
    if (fp2_x_rt < fp1_x_rt){
        sacc = -1;
    ...
    ct_array_rt.push(temp_delay_mod);
...
//     if (d.cell_id_x == fp1_x_rt && d.cell_id_y == fp1_y_rt) {
//         return "cell_rf1";
//     }
//     else if (d.cell_id_x == fp2_x_rt && d.cell_id_y == fp2_y_rt) {
//         return "cell_rf2";
//     }
//     else
//     {
...
// create a tooltip
var Tooltip = d3.select("#grid_right")
    .append("div")
    .style("opacity", 0)
    .attr("class", "tooltip")
...
function mouseover(d){
    //console.log('ct_array_rt-->'+ct_array_rt);
    let min = ct_array_rt[0];
    let max = ct_array_rt[0];
    for (i =0; i<ct_array_rt.length; i++){
        if (ct_array_rt[i]>max){
            max = ct_array_rt[i];
        }
        if (ct_array_rt[i]<min){
            min = ct_array_rt[i];
        }
        //console.log(ct_array_rt[i]);
    }
    //console.log('max-->'+max);
    //console.log('min-->'+min);
...
let svg_width_rt = 50*(x_range+1);
let svg_height_rt = 50*(y_range+1);



var grid = d3.select("#grid_right")
    .append("svg")
    .attr("id", "svg1")
    .attr("width",svg_width_rt)
    .attr("height",svg_height_rt);
